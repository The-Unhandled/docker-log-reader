...
class LogParser:
  ...
  private val logPattern: Regex = raw"\[(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3})\] \[(INFO|DEBUG|ERROR|WARNING)\] \[(\w+)\] (.+)".r

  def parse(log: String): Option[Log] = log match
    case logPattern(timestampStr, logLevelStr, module, message) =>
      val timestamp = LocalDateTime.parse(timestampStr, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss,SSS")) // Parse timestamp 
